...
func InitConnMgr() (err error) {
	...
	connMgr = &ConnMgr{
		buckets:      make([]*Bucket, G_config.BucketCount),// "连接分桶的数量": "桶越多, 推送的锁粒度越小, 推送并发度越高" "bucketCount": 512
		jobChan:      make([]chan *PushJob, G_config.BucketCount),
		dispatchChan: make(chan *PushJob, G_config.DispatchChannelSize),// "待分发队列的长度": "分发队列缓冲所有待推送的消息, 等待被分发到Bucket"  "dispatchChannelSize": 100000
	}
	for bucketIdx, _ = range connMgr.buckets {
		...
		// Bucket的Job worker
		for jobWorkerIdx = 0; jobWorkerIdx < G_config.BucketJobWorkerCount; jobWorkerIdx++ { // "Bucket发送协程的数量": "每个Bucket有多个协程并发的推送消息" "bucketJobWorkerCount": 32
			go connMgr.jobWorkerMain(jobWorkerIdx, bucketIdx)
		}
	}
	// 初始化分发协程, 用于将消息扇出给各个Bucket
	for dispatchWorkerIdx = 0; dispatchWorkerIdx < G_config.DispatchWorkerCount; dispatchWorkerIdx++ { // "分发协程的数量": "分发协程用于将待推送消息扇出给各个Bucket" "dispatchWorkerCount": 32
